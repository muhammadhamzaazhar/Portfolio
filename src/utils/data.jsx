...
import { VapiLogo, RetellLogo } from "../components/icons/brandLogos";
...
import work5AutomationCard from "../assets/himolla-multilingual-ai-phone-assistant-card.png";
import work5AutomationPic from "../assets/himolla-multilingual-ai-phone-assistant.png";
import work5AutomationDoc from "../assets/Himolla-Documentation.pdf";
import work6AutomationCard from "../assets/gastamo-outbound-ai-voice-agent-card.png";
import work6AutomationPic from "../assets/gastamo-outbound-ai-voice-agent.png";
import work6AutomationDoc from "../assets/Gastamo-Bot-Documentation.pdf";
import work7AutomationCard from "../assets/park-pizza-ai-ordering-agent-card.png";
import work7AutomationPic from "../assets/park-pizza-ai-ordering-agent.png";
import work7AutomationDoc from "../assets/Park_Pizza_Case_Study.pdf";
import work8AutomationCard from "../assets/ai-restaurant-ordering-payment-card.png";
import work8AutomationPic from "../assets/ai-restaurant-ordering-payment.png";
import work8AutomationDoc from "../assets/Roboto_Tokyo_Grill_Case_Study.pdf";
import work9AutomationCard from "../assets/ai-call-analytics-dashboard-card.png";
import work9AutomationPic from "../assets/ai-call-analytics-dashboard.png";
import work9AutomationDoc from "../assets/AI_Call_Analytics_Dashboard_Case_Study.pdf";
import goHighLevelIcon from "../assets/gohighlevel.webp";
...
export const iconsAutomation = [
  { icon1: <VapiLogo />, name: "Vapi" },
  { icon1: <RetellLogo />, name: "Retell AI" },
  { icon1: <SiN8N />, name: "n8n" },
  { icon1: <SiTwilio />, name: "Twilio" },
  { icon1: <BsDiagram3 />, name: "RAG" },
  { icon1: <SiLangchain />, name: "Langchain" },
  { icon1: <SiMake />, name: "Make" },
  { icon1: <img src={goHighLevelIcon} alt="GoHighLevel" />, name: "GoHighLevel" },
  { icon1: <SiCrewai />, name: "CrewAI" },
  { icon1: <SiOpenai />, name: "OpenAI" },
  { icon1: <SiClaude />, name: "Claude" },
  { icon1: <TbPrompt />, name: "Prompt Engineering" },
  { icon1: <FaNetworkWired />, name: "Multi Agent System" },
];
...
export const workImagesAI = [
  ...
  },
  {
    ...
    img: work6AI,
    name: "project 6",
    projectName: "Lungs Disease Detection",
    sourceCode:
      "https://github.com/muhammadhamzaazhar/Automated-Lungs-Disease-Detection-ChestXray14",
  },
  {
    id: 7,
    img: work7AI,
    name: "project 7",
    projectName: "Mitotic Cell Classification",
    sourceCode:
      "https://github.com/muhammadhamzaazhar/Explainable-System-For-Mitosis-Cell-Classification",
  },
];
...
export const workImagesAutomation = [
  {
    ...
    descPic: work7AutomationPic,
    documentation: work7AutomationDoc,
    name: "project 1",
    projectName: "Personalized Reservation & Ordering",
    description:
      "Park Pizza & Brewing Company, a local spot known for wood-fired pizzas and craft beer, was taking a growing volume of phone orders – and feeling the strain: order details collected differently by every staff member, no memory of returning customers, and no record of completed orders. We built Park, a conversational AI voice agent on Retell, orchestrated through n8n, that handles inbound orders the way an experienced team member would. The moment a call comes in, an n8n webhook looks the number up in a Google Sheets customer database – before Park even says hello – so returning callers are greeted by name and offered their last order in seconds. New customers are guided through a natural, one-question-at-a-time flow for pickup, delivery, or dine-in, with sizes, sauces, and toppings collected systematically so nothing is missed. Park books reservations by checking live availability, sends menu and confirmation links by SMS or email, and logs every completed order back to the sheet – building the history that powers the next call's recognition. The result: consistent, accurate orders on every call, regulars recognized instantly, and staff freed from the phone to focus on hospitality.",
    clientName: "Danny Marsh",
    designation: "Owner, Park Pizza & Brewing Company",
    feedback:
      "The part that still gets our regulars every time is that it knows them. Someone calls in and it greets them by name and offers their usual before they've even said what they want - that used to depend entirely on which staff member picked up. Now every order comes through the same way, sizes and toppings and all, nothing missed. It even handles our reservations and logs everything on its own. My team's back on the floor taking care of the people actually in the building instead of being stuck on the phone during a rush.",
  },
  {
    id: 2,
    img: work9AutomationCard,
    descPic: work9AutomationPic,
    documentation: work9AutomationDoc,
    name: "project 2",
    projectName: "AI Call Analytics Dashboard for Voice Agents",
    description:
      "Naura Agency was running multiple AI voice agents across different businesses and had no single place to see how any of them were performing – call volumes, durations, recordings, and transcripts were scattered, and analyzing agent effectiveness meant manual effort with no real-time view. We designed and built a complete AI Call Analytics Platform that brings everything into one dashboard. When a call ends, the Retell AI voice agent automatically pushes the call data through a webhook into a Supabase database, and a custom React dashboard processes and visualizes it instantly. From a single interface, businesses can track total calls, call duration, customer sentiment, and cost, play back recordings, and search transcripts – all updating in real time as new calls come in. The architecture is built to scale: new voice agents can be added without redesigning the platform. The result is full visibility into agent performance, faster decisions from instantly available metrics, and the elimination of manual reporting.",
    clientName: "Marcus Reeves",
    designation: "Founder - Naura Agency",
    feedback:
      "We were running voice agents for a handful of different businesses and had no real way to see what was happening across them - call volumes, costs, how customers were actually feeling on the calls. This dashboard pulled all of it into one place, updating on its own as calls come in. I can now open one screen and see exactly how every agent is performing, pull a recording or transcript in seconds, and spot a problem before a client even calls to ask. It turned a pile of scattered data into something we actually run the business on.",
  },
  {
    id: 3,
  ...
  {
    id: 5,
    img: work8AutomationCard,
    descPic: work8AutomationPic,
    documentation: work8AutomationDoc,
    name: "project 5",
    projectName: "AI Restaurant Ordering & Payment System",
    description:
      "Roboto Tokyo Grill runs multiple locations and takes a high volume of phone orders every day – which meant staff tied up on calls, mistakes from manual entry, fiddly modifier choices, and delays getting payment links out. We built a complete AI-powered ordering system that handles the whole call end to end. The voice assistant pulls a live, location-specific menu straight from Square, guides the customer through items and modifiers, and validates availability as it goes. Once the order is confirmed, the system creates it directly in Square, calculates totals and tax, generates a secure payment link, and sends it to the customer by SMS or email – along with delivery links for DoorDash or Uber Eats when needed. Orders are mapped to real Square catalog IDs, so accuracy is built in, and the whole flow runs without staff intervention. The result is faster service, fewer errors, automated payments, and a modern ordering experience that scales to far more orders without adding headcount.",
    clientName: "Kevin Tran",
    designation: "Owner, Roboto Tokyo Grill",
    feedback:
      "Phone orders were eating up our team during every rush - someone always stuck on the line instead of helping in-house, and the mistakes added up fast. Now the AI takes the order, pulls the right menu for whichever location the customer wants, gets the modifiers right, and texts them a payment link before they even hang up. Orders come through clean and already paid for. Across our Tallahassee locations it's let us handle way more volume without putting another person on the phones.",
  },
  {
    id: 6,
  ...
  {
    id: 8,
    img: work5AutomationCard,
    descPic: work5AutomationPic,
    documentation: work5AutomationDoc,
    name: "project 8",
    projectName: "Front Desk Agent for Furniture Brands",
    description:
    ...
    clientName: "Andreas Bruckner",
    designation: "Head of Customer Service, Himolla Polstermöbel GmbH",
    feedback:
  ...
  {
    id: 9,
    img: work6AutomationCard,
    descPic: work6AutomationPic,
    documentation: work6AutomationDoc,
    name: "project 9",
    projectName: "Direct Hire Recruitment Automation",
    description:
      "Gastamo, a Polish recruitment agency placing Polish-speaking professionals into permanent roles across Germany, Austria, and Switzerland, was generating inbound leads through Facebook forms but losing momentum on the follow-up – every discovery call was handled differently, low-intent leads ate up sales time, and the flagship offer wasn't always pitched consistently. We built Kamil, a German-language AI outbound voice agent on VAPI with an n8n automation backend. Kamil calls every Facebook lead automatically, qualifies it with a natural sequence of questions – which roles, how many people, and by when – delivers a confident company introduction, presents the partnership offer, and triggers a follow-up email with candidate profiles before the call ends. Unanswered leads are re-queued automatically and retried up to nine times with tracked timestamps, so nothing slips through. Every call returns structured data – profession, headcount, start date, and a summary – and handles common objections consistently. The result: the cold discovery call runs entirely on its own, and the human team only steps in for leads that are already warm and qualified.",
    clientName: "Tomasz Wójcik",
    designation: "Sales Manager, Gastamo",
    feedback:
      "Our Facebook leads used to sit until someone had time to call, and by then half of them had gone cold. Now every single lead gets called the moment it comes in, qualified properly, and pitched the same way every time. By the time one of my reps picks up the phone, the person already knows who we are and what we offer. It's freed the team to do what they're best at - closing - instead of repeating the same opening questions all day.",
  },
];
